...
class AddLambdaNaiveBayesTrainer[Label, Feature, Value](lambda: Double, fe: FeatureExtender[Feature, Value])
  ...
  def train(instances: Vector[(Label, Vector[(Feature, Value)])]): NaiveBayesModelToImplement[Label, Feature, Value] = {
    ...
    val featureAllValues = lfvToFv(instances)

    val pValuePlusLambda = featureLabelValues.map {
      case (feature, labelValues) =>
        (feature, new ConditionalProbabilityDistributionWithAllValues[Label, Value](labelValues, featureAllValues(feature), lambda))
    }
